...
driver.get('https://iframetester.com/') #website containing iframe

time.sleep(2)
iframe = driver.find_element(By.XPATH, value='//*[@id="iframe-window"]') #locating our iframe
driver.switch_to.frame(iframe) #switching to our iframe

print(iframe.tag_name, iframe.text) #printing both title of web_site and that of the iframe

time.sleep(4)
"""
basically locating iframe elements is similar, so you just yave to locate the iframe element , then you start narrowing you self down into specific element found int he iframe"
"""
driver.quit()